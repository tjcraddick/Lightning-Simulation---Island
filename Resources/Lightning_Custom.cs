...
public class Lightning_Custom : MonoBehaviour
{
    ...
    public void Initiate(GameObject start_obj_in, Vector3 start_pos_in,
        ...
        bool auto_in, bool offshoot_in, bool arc_in, List<GameObject> list_in)
    {
    ...
    void generate_Bolt(Vector3 startpos, Vector3 endpos, int num_generations)
    {
        ...
        for (int i = 0; i < num_generations; i++)
        {
            ...
            for (int j = curr_index; j < next_index; j++)
            {
                ...
                Dist = end + start; // + for avg?
                mid = Dist * 0.5f;

                offset_vec = vector_offset(start, end, offset, j);

                //randomly adjust this over duration if possible
                //for moving lightning
                mid = mid + offset_vec;

                //seg_lines[j].Midpoint.Set(mid.x, mid.y, mid.z);

                //Debug.Log("current index: " + curr_index);
                //Debug.Log("num segments: " + seg_lines.Count);
